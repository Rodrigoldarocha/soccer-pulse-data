...
export interface ApiJsonError {
  code: string;
  message: string;
  /** Optional extra details, e.g. API-specific error codes */
  details?: unknown[];
}

export type ApiJsonResult<T> = { success: true; data: T } | { success: false; error: ApiJsonError };

// ─── Shared JSON client (rate-limit aware) ───────────────────────────
const responseCache = new Map<string, { data: unknown; timestamp: number }>();
const inflight = new Map<string, Promise<ApiJsonResult<unknown>>>();
...
export async function apiJson<T>(
  ...
  if (pending) return pending as Promise<ApiJsonResult<T>>;

  const token = getToken();
  const headers: Record<string, string> = { Accept: "application/json" };
  if (token) headers["Authorization"] = `Token ${token}`;

  const run = async (): Promise<ApiJsonResult<T>> => {
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (res.status === 429 || res.status >= 500) {
          ...
          return {
            success: false,
            error: { code: "api_error", message: "API error or rate limited" },
          };
        }
        if (!res.ok) {
        ...
        if (!text.trim().startsWith("{") && !text.trim().startsWith("[")) {
          ...
          return {
            success: false,
            error: { code: "parse_error", message: "Response does not contain JSON" },
          };
        }
        ...
        return {
          success: false,
          error: { code: "fetch_error", message: e instanceof Error ? e.message : String(e) },
        };
      }
    }
    return { success: false, error: { code: "max_retries", message: "All retry attempts failed" } };
  };

  const queued = chain.then<ApiJsonResult<T>, ApiJsonResult<T>>(run, (error) => {
    // Transform any non-API errors into a structured error result
    return { success: false, error: { code: "chain_error", message: String(error) } };
  });
  chain = queued.catch(() => undefined);
  const shared = queued as Promise<ApiJsonResult<unknown>>;
...
// Sem colisões de ID: cada ID aparece uma vez. Rótulos vêm de
// dynamicLeagueNames (API) primeiro, depois LEAGUE_NAMES.
...
export const LEAGUE_IDS: Record<string, string> = {
  ...
  "usl-championship": "57",
  veikkausliiga: "55",
  // alias explícito: NPL Queensland ≠ A-League (IDs distintos na API)
  "npl-queensland": "npl-qld",
};
...
async function ensureLeagueNames(): Promise<void> {
  ...
  const result = await apiJson<BzzoiroPaginated<{ id: number; name: string }>>(
    `leagues/?limit=200`,
    {
      ttlMs: 24 * 60 * 60 * 1000,
    },
  ...
  for (const l of result.data.results) dynamicLeagueNames.set(String(l.id), l.name);
...
export async function fetchEventsByDateRange(from: string, to: string): Promise<TsdbEvent[]> {
  ...
  while (pageUrl) {
    const result: ApiJsonResult<BzzoiroPaginated<BzzoiroEvent>> = await apiJson<
      BzzoiroPaginated<BzzoiroEvent>
    >(pageUrl, {
      ttlMs: 10 * 60 * 1000,
    });
    if (!result.success) break;
    if (!result.data.results) break;
    const page: BzzoiroPaginated<BzzoiroEvent> = result.data;
...
function parseScoreDistribution(
  score: BzzoiroPredictionV2["markets"]["score"],
): Array<{ score: string; p: number }> | null {
  if (!score) return null;
  if (Array.isArray(score.distribution)) {
    return score.distribution
      .map((d) => {
        const s = d.score ?? (d.home != null && d.away != null ? `${d.home}-${d.away}` : null);
        const p = d.p ?? d.prob;
        if (!s || typeof p !== "number" || !Number.isFinite(p)) return null;
        return { score: s, p: p > 1 ? p / 100 : p };
      })
      .filter((x): x is { score: string; p: number } => x != null)
      .slice(0, 30);
  }
  return null;
}
...
export interface ApiPrediction {
  ...
  /** model.confidence (0..1) da API */
  modelConfidence?: number | null;
  mostLikelyScore?: string;
  modelVersion?: string;
  /** Distribuição de placares se a API expuser markets.score.matrix */
  scoreDistribution?: Array<{ score: string; p: number }> | null;
}

// Percentual 0..100 → fração 0..1. Zero preservado; ausente/fora de faixa → null.
const pct = (v: number | undefined): number | null =>
  typeof v === "number" && Number.isFinite(v) && v >= 0 && v <= 100 ? +(v / 100).toFixed(4) : null;

const xgOrZero = (v: number | undefined): number =>
  typeof v === "number" && Number.isFinite(v) && v >= 0 && v <= 10 ? +v.toFixed(2) : 0;
...
export async function fetchApiPredictions(
  ...
  for (let p = 0; p < MAX_PAGES; p++) {
    const result = await apiJson<BzzoiroPredictionV2[] | BzzoiroPaginated<BzzoiroPredictionV2>>(
    ...
    for (const row of rows) {
      ...
      if (!mr) continue;
      const probHome = pct(mr.prob_home);
      const probDraw = pct(mr.prob_draw);
      const probAway = pct(mr.prob_away);
      if (probHome == null || probDraw == null || probAway == null) continue;
      out.set(String(row.event.id), {
        eventId: String(row.event.id),
        xgHome: xgOrZero(xg?.home),
        xgAway: xgOrZero(xg?.away),
        probHome,
        probDraw,
        probAway,
        probOver15: pct(row.markets?.over_under?.prob_over_15),
        probOver25: pct(row.markets?.over_under?.prob_over_25),
        probOver35: pct(row.markets?.over_under?.prob_over_35),
        probBtts: pct(row.markets?.btts?.prob_yes),
        modelConfidence:
          typeof row.model?.confidence === "number" &&
          row.model.confidence >= 0 &&
          row.model.confidence <= 1
            ? row.model.confidence
            : null,
        mostLikelyScore: row.markets?.score?.most_likely,
        modelVersion: row.model?.version,
        scoreDistribution: parseScoreDistribution(row.markets?.score),
      });
    }
...
export async function fetchLeagueStandings(leagueId: string): Promise<TsdbStanding[]> {
  ...
  const result = await apiJson<BzzoiroStandingsResponse>(`leagues/${tsdbId}/standings/`, {
    ttlMs: 60 * 60 * 1000,
  });
  if (!result.success || !result.data?.standings) return [];

  return result.data.standings.map((r) => ({
...
export async function fetchLeaguePastEvents(leagueId: string): Promise<TsdbEvent[]> {
  ...
  for (let p = 0; p < MAX_PAGES; p++) {
    const result = await apiJson<BzzoiroPaginated<BzzoiroEvent>>(
      `events/?league_id=${tsdbId}&status=finished&ordering=-date&limit=${limit}&offset=${offset}`,
      { ttlMs: 60 * 60 * 1000 },
    );
    if (!result.success || !result.data?.results) break;
    for (const ev of result.data.results.map(mapBzzoiroEvent)) {
      if (!seen.has(ev.idEvent)) {
        seen.add(ev.idEvent);
        allEvents.push(ev);
      }
    }
    if (!result.data.next || result.data.results.length < limit) break;
    offset += limit;
  }
...
export async function fetchTeamId(teamName: string): Promise<string | null> {
  ...
  if (!result.success || !result.data?.results?.length) return null;

  // Try exact match first
  const lower = teamName.toLowerCase();
  const exact = result.data.results.find((t) => t.name.toLowerCase() === lower);
  return String(exact?.id ?? result.data.results[0].id);
}
