...
export type ApiJsonResult<T> =
  | { success: true; data: T }
  | { success: false; error: ApiJsonError };
...
export async function apiJson<T>(
  ...
  if (cached && Date.now() - cached.timestamp < ttlMs) return { success: true, data: cached.data as T };
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      try {
        ...
        if (res.status === 429 || res.status >= 500) {
          ...
          return { success: false, error: { code: "api_error", message: "API error or rate limited" } };
        }
        if (!res.ok) {
        ...
        if (!text.trim().startsWith("{") && !text.trim().startsWith("[")) {
          ...
          return { success: false, error: { code: "parse_error", message: "Response does not contain JSON" } };
        }
        ...
        return { success: false, error: { code: "fetch_error", message: (e instanceof Error ? e.message : String(e)) } };
      }
    }
...
async function ensureLeagueNames(): Promise<void> {
  ...
  const result = await apiJson<BzzoiroPaginated<{ id: number; name: string }>>(`leagues/?limit=200`, {
    ttlMs: 24 * 60 * 60 * 1000,
  });
