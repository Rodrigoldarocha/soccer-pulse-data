...
export async function fetchApiPrediction(eventId: string): Promise<ApiPrediction | null> {
  ...
  const result = await apiJson<unknown>(`events/${eventId}/prediction/`, {
    ttlMs: 30 * 60 * 1000,
  });
  if (!result.success || !isObj(result.data)) return null;
  const markets = isObj(result.data.markets)
    ? result.data.markets
    : isObj(result.data.prediction)
...
export async function fetchLiveEvents(): Promise<LiveEvent[] | null> {
  const result = await apiJson<unknown>(`events/live/?limit=100`, { ttlMs: 30 * 1000 });
  if (!result.success || !result.data) return null;
  const pool: unknown[] = Array.isArray(result.data) ? result.data : arr(isObj(result.data) ? result.data.events : []);
...
export async function fetchEventHeader(eventId: string): Promise<EventHeader | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/`, { ttlMs: 5 * 60 * 1000 });
  if (!result.success || !isObj(result.data)) return null;
  const homeName = str(result.data.home_team) ?? null;
  const awayName = str(result.data.away_team) ?? null;
  if (!homeName || !awayName) return null;
  const hid = result.data.home_team_id;
  const aid = result.data.away_team_id;
  return {
    homeName,
    awayName,
    homeTeamId: typeof hid === "number" ? String(hid) : (str(hid) ?? null),
    awayTeamId: typeof aid === "number" ? String(aid) : (str(aid) ?? null),
    leagueName: str(result.data.league_name) ?? str(result.data.league) ?? null,
    kickoff: str(result.data.event_date) ?? null,
    status: str(result.data.status) ?? null,
    homeScore: num(result.data.home_score),
    awayScore: num(result.data.away_score),
  };
}
...
export async function fetchH2H(
  ...
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/h2h/`, { ttlMs: 60 * 60 * 1000 });
  if (!result.success) return null;
  const pool: unknown[] = Array.isArray(result.data)
    ? result.data
    : [
        ...arr(isObj(result.data) ? result.data.matches : []),
        ...arr(isObj(result.data) ? result.data.recent_matches : []),
        ...arr(isObj(result.data) ? (isObj(result.data.h2h) ? result.data.h2h.matches : []) : []),
        ...arr(isObj(result.data) ? result.data.results : []),
        ...arr(isObj(result.data) ? result.data.games : []),
      ];
  const matches = pool
    .map(toH2HMatch)
...
export async function fetchTeamForm(
  ...
  const result = await apiJson<unknown>(`teams/${teamId}/fixtures/?limit=10`, {
    ttlMs: 30 * 60 * 1000,
  });
  if (!result.success) return null;
  const pool: unknown[] = Array.isArray(result.data) ? result.data : arr(isObj(result.data) ? result.data.results : []);
...
export async function fetchLineups(eventId: string): Promise<Lineups | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/lineups/`, {
    ttlMs: 15 * 60 * 1000,
  });
  if (!result.success || !isObj(result.data)) return null;
  const rawStatus = str(result.data.lineup_status) ?? "";
  const status: Lineups["status"] =
    rawStatus === "confirmed"
      ...
      : rawStatus === "predicted"
        ? "predicted"
        : "unavailable";
  const sides = isObj(result.data.lineups) ? result.data.lineups : result.data;
...
export async function fetchMatchStats(eventId: string): Promise<StatRow[] | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/stats/`, { ttlMs: 5 * 60 * 1000 });
  if (!result.success) return null;
  ...
  if (isObj(result.data)) {
    const home = isObj(result.data.home) ? result.data.home : isObj(result.data.home_team) ? result.data.home_team : null;
    const away = isObj(result.data.away) ? result.data.away : isObj(result.data.away_team) ? result.data.away_team : null;
    if (!home || !away) return null;
    const rows: StatRow[] = [];
...
export async function fetchIncidents(eventId: string): Promise<MatchIncident[] | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/incidents/`, { ttlMs: 60 * 1000 });
  if (!result.success) return null;
  const pool: unknown[] = Array.isArray(result.data) ? result.data : arr(isObj(result.data) ? result.data.incidents : []);
  const rows = pool
    .map((r) => {
      if (!isObj(r)) return null;
      return {
...
export async function fetchBroadcasts(eventId: string): Promise<BroadcastInfo[] | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/broadcasts/`, {
    ttlMs: 60 * 60 * 1000,
  });
  if (!result.success) return null;
  const pool: unknown[] = Array.isArray(result.data) ? result.data : arr(isObj(result.data) ? result.data.results : []);
...
export async function fetchVenueMeta(eventId: string): Promise<VenueMeta | null> {
  if (!validEventId(eventId)) return null;
  const result = await apiJson<unknown>(`events/${eventId}/metadata/`, {
    ttlMs: 6 * 60 * 60 * 1000,
  });
  if (!result.success) return null;
  const venue =
    deepPick(result.data, ["venue", "stadium", "stadium_name", "arena"]) ??
    (isObj(result.data) && isObj(result.data.venue) ? (str(result.data.venue.name) ?? null) : null);
  const city = deepPick(result.data, ["city", "venue_city"]);
  const referee =
    deepPick(result.data, ["referee", "referee_name", "official"]) ??
    (isObj(result.data) && isObj(result.data.referee) ? (str(result.data.referee.name) ?? null) : null);
