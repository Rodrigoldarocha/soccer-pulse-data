...
export async function fetchLiveEvents(): Promise<LiveEvent[] | null> {
  ...
  const pool: unknown[] = Array.isArray(result.data)
    ? result.data
    : arr(isObj(result.data) ? result.data.events : []);
...
export async function fetchMatchStats(eventId: string): Promise<StatRow[] | null> {
  ...
  if (isObj(result.data)) {
    const home = isObj(result.data.home)
      ? result.data.home
      : isObj(result.data.home_team)
        ? result.data.home_team
        : null;
    const away = isObj(result.data.away)
      ? result.data.away
      : isObj(result.data.away_team)
...
export async function fetchIncidents(eventId: string): Promise<MatchIncident[] | null> {
  ...
  const pool: unknown[] = Array.isArray(result.data)
    ? result.data
    : arr(isObj(result.data) ? result.data.incidents : []);
  const rows = pool
    .map((r) => {
      if (!isObj(r)) return null;
      return {
...
export async function fetchVenueMeta(eventId: string): Promise<VenueMeta | null> {
  ...
  const referee =
    ...
    (isObj(result.data) && isObj(result.data.referee)
      ? (str(result.data.referee.name) ?? null)
      : null);
