...
export async function buildPrediction(
  ...
  opts?: { trustSource?: boolean; modelVersion?: string },
  ...
  if (pred) {
    ...
    // Official API numbers: exact passthrough, no calibration, no blend.
    // Zeros preserved; non-finite input rejected explicitly.
    const processed = opts?.trustSource
      ? allMarkets.map((mc) => {
          if (!Number.isFinite(mc.raw)) {
            throw new Error(`Invalid official probability for ${mc.market}: ${mc.raw}`);
          }
          const probability = Math.min(1, Math.max(0, mc.raw));
          return {
            market: mc.market,
            probability,
