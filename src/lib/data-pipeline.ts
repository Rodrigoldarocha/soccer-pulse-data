...
/** Converte a previsão da API para PredictionData (chamado só quando os campos existem). */
...
// ─── Indisponível (sem valores inventados) ────────────────────────────
// Partida continua visível; UI exibe "Previsão indisponível".
...
function unavailablePrediction(
  ...
  return {
    ...
    probabilities: { home: 0, draw: 0, away: 0, over25: 0, btts: 0 },
    odds: { home: 0, draw: 0, away: 0, over25: 0, btts: 0, doubleChance1X: 0 },
...
// ─── Snapshot imutável pré-jogo ─────────────────────────────────────────
// Somente agendados com previsão válida; primeira gravação vence.
...
async function snapshotPredictions(preds: MatchPrediction[]): Promise<void> {
  ...
  const jobs = preds.flatMap((p) => {
    ...
    const leagueId = Number(LEAGUE_IDS[p.league] ?? p.league);
    if (!Number.isFinite(eventId) || !Number.isFinite(leagueId)) return [];
    const rows: { market: MarketId; probability: number; odds: number }[] = [
      { market: "1X2_HOME", probability: p.probabilities.home, odds: p.odds.home },
      { market: "DRAW", probability: p.probabilities.draw, odds: p.odds.draw },
      { market: "1X2_AWAY", probability: p.probabilities.away, odds: p.odds.away },
      { market: "OVER_2_5", probability: p.probabilities.over25, odds: p.odds.over25 },
      { market: "BTTS", probability: p.probabilities.btts, odds: p.odds.btts },
    ];
    return rows.map((r) =>
      storePrediction({
        eventId,
...
async function runPipeline(dateISO?: string): Promise<MatchPrediction[]> {
  ...
  const results = await Promise.allSettled(
    events.map(async (ev) => {
      ...
      if (fromApi && fromApi.probOver25 != null && fromApi.probBtts != null) {
      ...
      try {
        ...
        return buildPrediction(footballEvent, prediction, leagueMeta, { trustSource: false });
      } catch {
        return unavailablePrediction(footballEvent, leagueMeta);
      }
    }),
  ...
  const results = await Promise.allSettled(
    live.slice(0, 20).map(async (ev) => {
      ...
      try {
        ...
        built = await buildPred(footballEvent, prediction, leagueMeta, { trustSource: false });
      } catch {
        return {
          ...unavailablePrediction(footballEvent, leagueMeta),
          minute: ev.minute ?? undefined,
  ...
  const results = await Promise.allSettled(
    predictionInputs.map(async (ev) => {
      ...
      try {
        ...
        return buildPred(footballEvent, prediction, leagueMeta, { trustSource: false });
      } catch {
        return unavailablePrediction(footballEvent, leagueMeta);
      }
    }),
