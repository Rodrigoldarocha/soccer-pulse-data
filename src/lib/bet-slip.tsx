...
export function BetSlipProvider({ children }: { children: ReactNode }) {
  ...
  const toggleLeg = useCallback((leg: SlipLeg) => {
    setLegs((prev) => {
      ...
      if (existing)
        return prev.filter((l) => !(l.matchId === leg.matchId && l.market === leg.market));
  ...
  const hasLeg = useCallback(
    (matchId: string, market: MarketId) =>
      legs.some((l) => l.matchId === matchId && l.market === market),
    [legs],
  );
...
export function riskLevel(probability: number, legCount: number) {
  if (legCount === 0)
    return {
      label: "Adicione seleções",
      tone: "muted" as const,
      hint: "Escolha jogos para montar sua múltipla.",
    };
  if (probability >= 0.55)
    return {
      label: "Excelente / Baixo Risco",
      tone: "safe" as const,
      hint: "Combinação matematicamente sólida.",
    };
  if (probability >= 0.3)
    return {
      label: "Bom valor / Risco Moderado",
      tone: "moderate" as const,
      hint: "Equilíbrio saudável entre odds e probabilidade.",
    };
  if (probability >= 0.12)
    return {
      label: "Alto Risco",
      tone: "aggressive" as const,
      hint: "Odds atraentes, mas a chance real é limitada.",
    };
  if (probability >= 0.04)
    return {
      label: "Muito Ousada",
      tone: "danger" as const,
      hint: "Cada perna adicional multiplica o risco.",
    };
  return {
    label: "Jackpot Extremo",
    tone: "extreme" as const,
    hint: "Considere reduzir o número de pernas para melhorar a chance real.",
  };
}
