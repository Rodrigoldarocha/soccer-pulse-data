...
function computeXg(
  ...
  // Média da liga calculada dos dados (não fixa). Fallback 1.3 só sem histórico.
  const leagueAvg = leagueAvgGoals && leagueAvgGoals > 0.4 ? leagueAvgGoals : 1.3;
...
export async function generatePredictions(dateISO?: string): Promise<PredictionInput[]> {
  // Sempre fuso America/Sao_Paulo — nunca UTC direto (D9).
  const dateStr = dateISO ?? spTodayISO();
...
export function invalidateTeamForms(): void {
  // no-op: cache is now snapped per-call; clearing has no effect on in-flight predictions
}
...
export async function computePrediction(
  ...
  // Cache snapshotted at call time to avoid race conditions in SSR concurrent requests.
  const cachedHomeForm = teamFormCache.get(normalizeTeamName(homeTeam));
  const cachedAwayForm = teamFormCache.get(normalizeTeamName(awayTeam));

  if (cachedHomeForm && cachedHomeForm.games >= 3) {
    effectiveHome.avgGoalsScored = (effectiveHome.avgGoalsScored + cachedHomeForm.scored) / 2;
    effectiveHome.avgGoalsConceded = (effectiveHome.avgGoalsConceded + cachedHomeForm.conceded) / 2;
  }
  if (cachedAwayForm && cachedAwayForm.games >= 3) {
    effectiveAway.avgGoalsScored = (effectiveAway.avgGoalsScored + cachedAwayForm.scored) / 2;
    effectiveAway.avgGoalsConceded = (effectiveAway.avgGoalsConceded + cachedAwayForm.conceded) / 2;
  }

  // Média de gols da liga calculada dos dados históricos (não 1.3 fixo)
  let leagueAvgGoals: number | undefined;
  if (events.length > 0) {
    let gf = 0;
    let n = 0;
    for (const ev of events) {
    ...
    if (n > 0) leagueAvgGoals = +(gf / n / 2).toFixed(3);
  }

  // Compute xG
  const { xgHome, xgAway } = computeXg(effectiveHome, effectiveAway, leagueAvgGoals);
