...
import {
  getRealMatches,
  getRealLiveMatches,
  getUpcomingMatches as fetchUpcoming,
  getCachedOrGenerate,
} from "./matches.server";
import { spDateISO } from "./match-dates";
...
export const getTodayMatches = createServerFn({ method: "GET" }).handler(async () => {
  try {
    const date = todayISO();
    const matches = await getCachedOrGenerate(`today:${date}`, 60 * 15, () => getRealMatches(date));
    return { date, matches };
  } catch (error) {
    console.error("[getTodayMatches]", error);
    return { date: spDateISO(0), matches: [] };
  }
});
