...
import { ArrowLeft, Trophy, Shirt, BarChart3, Radio, Tv, MapPin, Target, Info } from "lucide-react";
...
function MatchPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {card && card.predictionStatus !== "unavailable" && (
        <Section icon={Target} title="Palpite">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded border border-border/50 bg-white/[0.02] px-3 py-2">
              ...
              <p className="mt-0.5 font-display text-sm font-bold text-foreground">
                {card.headlineLabel ?? MARKET_LABELS[card.suggestedMarket] ?? card.suggestedMarket}
              </p>
            </div>
            <div className="rounded border border-border/50 bg-white/[0.02] px-3 py-2">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground/50">Prob</p>
              <p className="mt-0.5 font-display text-sm font-bold tabular-nums text-foreground">
                {Math.round((card.headlineProbability ?? card.suggestedProbability) * 100)}%
              </p>
            </div>
            <div className="rounded border border-border/50 bg-white/[0.02] px-3 py-2">
              <p className="text-[10px] uppercase tracking-wider text-muted-foreground/50">Odd</p>
              <p className="mt-0.5 font-display text-sm font-bold tabular-nums text-foreground">
                {card.oddsAvailable ? (card.headlineOdds ?? card.suggestedOdds).toFixed(2) : "—"}
              </p>
            </div>
          </div>
          <p className="mt-2 text-[11px] text-muted-foreground/50">
            {card.confidenceReason ?? "Hierarquia visual — não é recomendação de aposta."}
          </p>
              ...
              <table className="w-full min-w-[420px] text-left text-xs tabular-nums">
                <thead>
                  <tr className="text-[10px] uppercase tracking-wider text-muted-foreground/50">
                    <th className="pb-1 font-medium">Mercado</th>
                    <th className="pb-1 font-medium">Prob</th>
                    <th className="pb-1 font-medium">Odd real</th>
                    <th className="pb-1 font-medium">Fair</th>
                    <th className="pb-1 font-medium">EV</th>
                  </tr>
                </thead>
                <tbody>
                  {card.markets
                    .filter((m) => m.market !== "AH_HOME_M05" && m.market !== "AH_AWAY_P05")
                    .slice(0, 12)
                    .map((m) => {
                      const ev = m.odd != null && m.odd > 1 ? m.probability * m.odd - 1 : null;
                      return (
                        <tr
                          key={m.market}
                        ...
                        >
                          ...
                          <td className="py-1.5 pr-2">{Math.round(m.probability * 100)}%</td>
                          <td className="py-1.5 pr-2">{m.odd != null ? m.odd.toFixed(2) : "—"}</td>
                          <td className="py-1.5 pr-2">{m.fairOdds.toFixed(2)}</td>
                          <td
                            className={cn(
          ...
          <p className="mt-2 flex items-start gap-1 text-[11px] text-muted-foreground/40">
            <Info className="mt-0.5 h-3 w-3 shrink-0" />
            Fair = 1/p do modelo (sem margem). Odd real ausente = “sem odd”. +18 · Jogue com
            responsabilidade.
          </p>
        </Section>
      )}
