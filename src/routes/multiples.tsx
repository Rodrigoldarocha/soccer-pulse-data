      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <div className="text-[10px] font-medium text-muted-foreground">
            {(s.totalProbability * 100).toFixed(1)}% real
          </div>
        </div>
      </div>
...
function MultiplesPage() {
  ...
  const { data: aiData } = useSuspenseQuery(
    queryOptions({ queryKey: ["ai-suggest"], queryFn: ai }),
  );
  const { data: todayData } = useSuspenseQuery(
    queryOptions({ queryKey: ["today"], queryFn: today }),
  );
