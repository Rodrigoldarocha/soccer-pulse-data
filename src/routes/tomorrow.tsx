  ...
  return (
    <div>
      <header className="mb-5">
        <h1 className="font-display text-2xl font-bold text-foreground sm:text-3xl">Amanhã</h1>
      ...
      <div className="relative mb-5">
        <Search
          className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/30"
          aria-hidden="true"
        />
        <input
          value={q}
        ...
        {filtered.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground/60">
              {q ? `Nenhum resultado para "${q}"` : "Nenhuma partida encontrada para amanhã."}
            </p>
            {q && (
              <button
                onClick={() => setQ("")}
